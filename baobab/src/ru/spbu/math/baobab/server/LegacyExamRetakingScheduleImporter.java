...
/**
 * This class imports exam retaking schedule from the following format: one file with
 ...
 * 
 * #<attendee_id_list>    -- #151
 * <topic name>           -- Математический анализ
 * <owner name>           -- Доц. Флоринский А.А.
 * <date>                 -- 11.02, 15.02*, 18.02, 22.02*
 * <auditorium_id>        -- 4519
 *  
 ...
 * 
 * If a date ends with "*" then it's exam committee, in other case it's exam retaking.
 *  
 * @author dbarashev, agudulin
 */
class LegacyExamRetakingScheduleImporter {
   ...
   */
  LegacyExamRetakingScheduleImporter(AuditoriumExtent auditoriumExtent, AttendeeExtent attendeeExtent, TopicExtent topicExtent) {
  ...
  private void createEvent(Topic topic, Auditorium auditorium, List<Attendee> groups, String dateLine,
    ...
    String[] dates = dateLine.replaceAll("[ \t]+", "").split(",");
    for (String date : dates) {
      String examType = "пересдача";
      if (date.contains("*")) {
        examType = "комиссия";
        date = date.replace("*", "");
      }
      //TODO: add timeLine info to the topic 
      String command = String.format("событие \"%s\" состоится на \"%s\" %s в \"%s\" для %s", topic.getID(),
          examType, convertDate(date), auditorium.getID(), createAttendeeList(groups));
