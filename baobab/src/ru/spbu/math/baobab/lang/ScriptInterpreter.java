...
public class ScriptInterpreter {
  ...
  private final static String DEFINE_TIMESLOT = String.format(
      "((?<%s>define\\stimeslot)\\s(?<%s>%s)\\s(?<%s>%s)\\sto\\s(?<%s>%s)\\son\\s(?<%s>%s)\\s(?<%s>%s))",
      DEFINE_TIMESLOT_NAME, DEFINE_TIMESLOT_NAME + ID_NAME, ID_PATTERN, DEFINE_TIMESLOT_NAME + TIME_NAME, TIME_PATTERN,
      DEFINE_TIMESLOT_NAME + TIME_NAME + "2", TIME_PATTERN, DEFINE_TIMESLOT_NAME + EVEN_ODD_NAME, EVEN_ODD_PATTERN_ENG,
      DEFINE_TIMESLOT_NAME + WEEKDAY_NAME, WEEKDAY_PATTERN_ENG)
      + String.format(
          "|((?<%s>определить\\sинтервал)\\s(?<%s>%s)\\sот\\s(?<%s>%s)\\sдо\\s(?<%s>%s)\\sв\\s(?<%s>%s)\\s(?<%s>%s))",
          DEFINE_TIMESLOT_NAME + RUSSIAN_VERSION, DEFINE_TIMESLOT_NAME + ID_NAME + RUSSIAN_VERSION, ID_PATTERN,
          DEFINE_TIMESLOT_NAME + TIME_NAME + RUSSIAN_VERSION, TIME_PATTERN, DEFINE_TIMESLOT_NAME + TIME_NAME
              + RUSSIAN_VERSION + "2", TIME_PATTERN, DEFINE_TIMESLOT_NAME + EVEN_ODD_NAME + RUSSIAN_VERSION,
          EVEN_ODD_PATTERN_RUS, DEFINE_TIMESLOT_NAME + WEEKDAY_NAME + RUSSIAN_VERSION, WEEKDAY_PATTERN_RUS);
  private final static String DEFINE_ATTENDEE = "";

  private static Pattern MAIN_PATTERN = Pattern.compile("^" + DEFINE_TIMESLOT + "$");
  ...
  public void process(String command) throws Exception {
    ...
    if (commandMatch.matches()) {
      ...
      throw new Exception("Exception: this command doesn't correspond to a pattern");
    }
  }
  ...
  private void execute(Matcher match) throws Exception {
    if ((match.group(DEFINE_TIMESLOT_NAME) != null) || (match.group(DEFINE_TIMESLOT_NAME + RUSSIAN_VERSION) != null)) {
  ...
  private void defineTimeSlot(Matcher match) throws Exception {
    String id = match.group(DEFINE_TIMESLOT_NAME + ID_NAME) != null ? match.group(DEFINE_TIMESLOT_NAME + ID_NAME)
        : match.group(DEFINE_TIMESLOT_NAME + ID_NAME + RUSSIAN_VERSION);
    TimeInstant start = TimeInstantConverter
        .convertToTimeInstant(match.group(DEFINE_TIMESLOT_NAME + TIME_NAME) != null ? match.group(DEFINE_TIMESLOT_NAME
            + TIME_NAME) : match.group(DEFINE_TIMESLOT_NAME + TIME_NAME + RUSSIAN_VERSION));
    TimeInstant finish = TimeInstantConverter
        .convertToTimeInstant(match.group(DEFINE_TIMESLOT_NAME + TIME_NAME + "2") != null ? match
            .group(DEFINE_TIMESLOT_NAME + TIME_NAME + "2") : match.group(DEFINE_TIMESLOT_NAME + TIME_NAME
            + RUSSIAN_VERSION + "2"));
    EvenOddWeek flashing = OddEvenWeekConverter
        .convertToOddEvenWeek(match.group(DEFINE_TIMESLOT_NAME + EVEN_ODD_NAME) != null ? match
            .group(DEFINE_TIMESLOT_NAME + EVEN_ODD_NAME) : match.group(DEFINE_TIMESLOT_NAME + EVEN_ODD_NAME
            + RUSSIAN_VERSION));
    Integer weekday = WeekdayConverter.convertToInt(match.group(DEFINE_TIMESLOT_NAME + WEEKDAY_NAME) != null ? match
