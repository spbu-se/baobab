...
public abstract class Parser {
  ...
  public static final List<String> RETAKING = Arrays.asList("retaking", "пересдача");
  public static final List<String> COMMITTEE = Arrays.asList("committee", "комиссия");
  ...
  protected final static String TOPIC_TYPE_PATTERN_ENG = String.format("%s|%s|%s|%s|%s|%s|%s|%s|%s", LECTURE_COURSE.get(0),
      LABS_COURSE.get(0), EXAM.get(0), INVITED_LECTURE.get(0), TEAM_MEETING.get(0), THESIS_DEFENSE.get(0), OFFICE_HOURS.get(0),
      RETAKING.get(0), COMMITTEE.get(0));
  protected final static String TOPIC_TYPE_PATTERN_RUS = String.format("%s|%s|%s|%s|%s|%s|%s|%s|%s", LECTURE_COURSE.get(1),
      LABS_COURSE.get(1), EXAM.get(1), INVITED_LECTURE.get(1), TEAM_MEETING.get(1), THESIS_DEFENSE.get(1), OFFICE_HOURS.get(1),
      RETAKING.get(1), COMMITTEE.get(1));
