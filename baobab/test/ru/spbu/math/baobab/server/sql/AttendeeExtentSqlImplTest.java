...
public class AttendeeExtentSqlImplTest extends SqlTestCase {
  ...
  public void testCreateNonGroupMembers() {
    ...
    expectSql("SELECT id uid name type group_id FROM Attendee WHERE uid").withParameters(1, "student");
    ...
    try {
      expectSql("SELECT id uid name type group_id FROM Attendee WHERE uid").withParameters(1, "student").withResult(
  ...
  public void testCreateGroupMembers() {
    AttendeeExtent attendeeExtent = new AttendeeExtentSqlImpl();
    // create group
    expectSql("SELECT id uid name type group_id FROM Attendee WHERE uid").withParameters(1, "group1");
  ...
  public void testFind() {
    ...
    expectSql("SELECT id uid name type group_id FROM Attendee WHERE uid").withParameters(1, "blablabla");
    assertNull(attendeeExtent.find("blablabla"));
    // test find method when attendee with the given id exists
    expectSql("SELECT id uid name type group_id FROM Attendee WHERE uid").withParameters(1, "teacher").withResult(
